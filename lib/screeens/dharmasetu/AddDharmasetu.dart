...
class _AddDharmaSetuState extends State<AddDharmasetu> {
  ...
  // For Jeevanadi Members search
  final _jeevanaadiSearchController = TextEditingController();
  bool _showJeevanaadiSuggestions = false;
  final FocusNode _jeevanaadiSearchFocusNode = FocusNode();
  final ScrollController _jeevanaadiScrollController = ScrollController();
  ...
  void initState() {
    ...
    // Setup listeners for Jeevanadi search
    _jeevanaadiSearchFocusNode.addListener(() {
      if (_jeevanaadiSearchFocusNode.hasFocus && _jeevanaadiSearchController.text.isEmpty) {
        setState(() => _showJeevanaadiSuggestions = true);
      }
    });
  }
  ...
  void dispose() {
    ...
    _dateController.dispose();
    _referredByController.dispose();
    _jeevanaadiSearchController.dispose();
    _jeevanaadiSearchFocusNode.dispose();
    _jeevanaadiScrollController.dispose();
    super.dispose();
  }

  void _performJeevanaadiSearch(String query) {
    if (query.trim().isEmpty) {
      setState(() => _showJeevanaadiSuggestions = false);
      return;
    }
                      ...
                      child: Form(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            // Date and Jeevanadi Members
                            _twoFieldRow(
                              _dateField(),
                              _buildJeevanaadiMembersField(),
                            ),
  ...
  // Jeevanadi Members search field with SearchJeevanaadiUsersEvent integration
 // Jeevanadi Members search field with SearchJeevanaadiUsersEvent integration
Widget _buildJeevanaadiMembersField() {
  return BlocBuilder<JeevanaadiBloc, JeevanaadiState>(
    builder: (context, state) {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          TextFormField(
            controller: _jeevanaadiSearchController,
            focusNode: _jeevanaadiSearchFocusNode,
            enabled: !_isSubmitting,
            ...
            decoration: InputDecoration(
              ...
              filled: true,
              fillColor: Colors.white,
              suffixIcon: state.searchLoading
                  ? Container(
                  ...
                  : IconButton(
                      ...
                      onPressed: () {
                        if (_jeevanaadiSearchController.text.trim().isNotEmpty) {
                          _performJeevanaadiSearch(_jeevanaadiSearchController.text);
                        }
                      },
                      color: Colors.brown,
                    ),
            ...
            validator: (v) =>
                _selectedReferredBy == null && (v == null || v.trim().isEmpty)
                    ? 'Please select a member'
                    : null,
          ),
          const SizedBox(height: 4),
          if (_showJeevanaadiSuggestions)
            Container(
              ...
              child: state.searchResults.isEmpty && !state.searchLoading
                  ? Padding(
                      padding: const EdgeInsets.all(16),
                      child: Column(
                        children: [
                  ...
                  : ListView.builder(
                      controller: _jeevanaadiScrollController,
                      shrinkWrap: true,
                      itemCount: state.searchResults.length +
                          (state.searchLoading ? 1 : 0),
                      itemBuilder: (context, index) {
                        ...
                        return _buildJeevanaadiUserTile(user);
                      },
                    ),
            ),
        ],
  ...
  Widget _buildJeevanaadiUserTile(JeevanaadiUser user) {
    return ListTile(
      ...
      onTap: () {
        setState(() {
          ...
          _jeevanaadiSearchController.text = user.fullName;
          _referredByController.text = user.fullName;
          _showJeevanaadiSuggestions = false;
        });
