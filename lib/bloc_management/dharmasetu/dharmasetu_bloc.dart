...
import 'package:vikas_app/api_services/network_repos/dharmasetu_repository.dart';
...
class DharmasetuBloc extends Bloc<DharmasetuEvent, DharmasetuState> {
  final DharmasetuRepository dharmasetuRepository;

  DharmasetuBloc({required this.dharmasetuRepository}) : super(DharmasetuState()) {
      ...
      if (response != null && response['content'] != null && response['content'] is List) {
        viewList.addAll(
          (response['content'] as List)
              .map((json) => DharmasetuView.fromJson(json as Map<String, dynamic>))
              .toList()
        );
      }
 ...
 Future<void> _onAddDharmasetu(
    ...
    try {
      ...
      final dharmasetuJson = event.dharmasetu.toJson(); // Use the model's toJson method

      print('Sending to API: $dharmasetuJson'); // Debug log

      final result = await dharmasetuRepository.createDharmasetu(dharmasetuJson);
  ...
  Future<void> _onUpdateDharmasetu(
    ...
    try {
      ...
      if (!result.isSuccess) {
        emit(state.copyWith(
          isSubmitting: false,
          errorMessage: result.error?.message ?? 'Failed to update dharmasetu',
        ));
        return;
      }
  ...
  Future<void> _onDeleteDharmasetu(
    ...
    try {
      ...
      if (!result.isSuccess) {
        emit(state.copyWith(
          isDeleting: false,
          errorMessage: result.error?.message ?? 'Failed to delete dharmasetu',
        ));
        return;
      }
